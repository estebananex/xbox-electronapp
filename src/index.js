...
function openPopup() {
  ...
  popupWindow.loadFile('./extensions/regionchanger/popup.html');
}
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    const extensionPath = '../extensions/regionchanger';
    session.defaultSession.loadExtension(path.resolve(__dirname, extensionPath)).then(({ id }) => {
      // ...
    })
    createWindow();
